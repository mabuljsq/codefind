...
# Constants
NOTIFICATION_MESSAGE = "CodeFind is waiting for your input"
...
class InputOutput:
    ...
    def __init__(
        ...
        self.append_chat_history(f"\n# codefind chat started at {current_time}\n\n")
...
{prefix} {hist}"""
    ...
    def offer_url(self, url, prompt="Open URL for more info?", allow_never=True):
        """URL opening disabled - CodeFind focuses on Bedrock and knowledge graphs."""
        # Just display the URL instead of offering to open it
        self.tool_output(f"URL: {url}")
        return False
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":  # macOS
            ...
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'CodeFind' -message '{NOTIFICATION_MESSAGE}'"
            # Fall back to osascript
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "CodeFind"\''
            )
        elif system == "Linux":
            ...
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'CodeFind' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
