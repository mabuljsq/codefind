...
from codefind import __version__
from codefind.args_formatter import (
    DotEnvFormatter,
...
def resolve_codefindignore_path(path_str, git_root=None):
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        description="codefind is AI pair programming with AWS Bedrock and knowledge graphs",
        add_config_file_help=True,
        default_config_files=default_config_files,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
        auto_env_var_prefix="CODEFIND_",
    )
    ...
    from codefind import coders as _codefind_coders

    edit_format_choices = sorted(
        {
            c.edit_format
            for c in _codefind_coders.__all__
    ...
    group.add_argument(
        ...
        help="Set an environment variable (to control AWS settings, can be used multiple times)",
        default=[],
    )
    ...
    group.add_argument(
        ...
        default=".codefind.model.settings.yml",
        help="Specify a file with CodeFind model settings for unknown models",
    ).complete = shtab.FILE
    group.add_argument(
        "--model-metadata-file",
        metavar="MODEL_METADATA_FILE",
        default=".codefind.model.metadata.json",
    ...
    default_input_history_file = (
        os.path.join(git_root, ".codefind.input.history") if git_root else ".codefind.input.history"
    )
    default_chat_history_file = (
        os.path.join(git_root, ".codefind.chat.history.md") if git_root else ".codefind.chat.history.md"
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .codefind.llm.history)",
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Print detailed API response including token usage information (default: False)",
    )
    group.add_argument(
        "--user-input-color",
        default="#00cc00",
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .codefind* to .gitignore (default: True)",
    )
    group.add_argument(
        "--add-gitignore-files",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable/disable the addition of files listed in .gitignore to CodeFind's editing scope.",
    )
    default_codefindignore_file = (
        os.path.join(git_root, ".codefindignore") if git_root else ".codefindignore"
    )

    group.add_argument(
        "--codefindignore",
        metavar="CODEFINDIGNORE",
        type=lambda path_str: resolve_codefindignore_path(path_str, git_root),
        default=default_codefindignore_file,
        help="Specify the codefind ignore file (default: .codefindignore in git root)",
    ).complete = shtab.FILE
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute codefind code changes in the git author name (default: True). If explicitly set"
    ...
    group.add_argument(
        ...
        default=None,
        help=(
            "Attribute codefind commits in the git committer name (default: True). If explicitly set"
            " to True, overrides --attribute-co-authored-by precedence for codefind edits."
        ),
    )
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'codefind: ' if codefind authored the changes (default: False)",
    )
    group.add_argument(
        "--attribute-commit-message-committer",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix all commit messages with 'codefind: ' (default: False)",
    )
    group.add_argument(
        ...
        default=True,
        help=(
            "Attribute codefind edits using the Co-authored-by trailer in the commit message"
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help="Check for new codefind versions on launch",
        default=True,
    )
    group.add_argument(
        "--show-release-notes",
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help="Upgrade codefind to the latest version from PyPI",
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    # GUI/browser mode removed - CodeFind focuses on command-line Bedrock integration
    group.add_argument(
        "--copy-paste",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable automatic copy/paste of chat between codefind and web UI (default: False)",
    )
    group.add_argument(
        "--apply",
        metavar="FILE",
    ...
    # Voice settings removed - CodeFind focuses on text-based Bedrock integration

    ######
    group = parser.add_argument_group("Other settings")
    # Playwright removed - CodeFind focuses on Bedrock and knowledge graphs
    group.add_argument(
        "--file",
        action="append",
        metavar="FILE",
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .codefind.conf.yml in git root, cwd"
            " or home directory)"
        ),
    ).complete = shtab.FILE
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable dynamic .env file discovery (default: True)",
    )
    group.add_argument(
        "--env-discovery-verbose",
        action="store_true",
        default=False,
        help="Show detailed information about .env file discovery process",
    )
    group.add_argument(
    ...
    ##########
    group = parser.add_argument_group("AWS Bedrock Knowledge Graph")
    group.add_argument(
        "--bedrock-knowledge-base-id",
        metavar="KNOWLEDGE_BASE_ID",
        env_var="BEDROCK_KNOWLEDGE_BASE_ID",
        help="AWS Bedrock Knowledge Base ID for GraphRAG retrieval",
    )
    group.add_argument(
        "--bedrock-region",
        metavar="REGION",
        ...
        default="us-west-2",
        help="AWS region for Bedrock Knowledge Base (default: us-west-2)",
    )
    group.add_argument(
        "--bedrock-max-results",
        type=int,
        metavar="MAX_RESULTS",
        env_var="BEDROCK_MAX_RESULTS",
        default=10,
        help="Maximum number of results to retrieve from knowledge graph (default: 10)",
    )
    group.add_argument(
        "--bedrock-search-type",
        choices=["HYBRID", "SEMANTIC"],
        env_var="BEDROCK_SEARCH_TYPE",
        default="HYBRID",
        help="Search type for knowledge graph retrieval (default: HYBRID)",
    )
    group.add_argument(
        "--bedrock-enable-reranking",
        action="store_true",
        default=False,
        help="Enable reranking of knowledge graph results (default: False)",
    )
    group.add_argument(
        "--bedrock-reranking-model-arn",
        metavar="MODEL_ARN",
        help="ARN of the reranking model to use with knowledge graph",
    )
    group.add_argument(
        "--bedrock-guardrail-id",
        metavar="GUARDRAIL_ID",
        help="Guardrail ID for knowledge graph queries",
    )
    group.add_argument(
        "--bedrock-guardrail-version",
        metavar="GUARDRAIL_VERSION",
        help="Guardrail version for knowledge graph queries",
    )
    ...
    group.add_argument(
        ...
        help=(
            ...
            f" {', '.join(supported_shells_list)}. Example: codefind --shell-completions bash"
        ),
    )

    # Deprecated model settings removed - CodeFind only supports current Bedrock models
    ...
    if command == "md":
        ...
        if len(sys.argv) > 2:
            ...
            if shell not in shtab.SUPPORTED_SHELLS:
                ...
                sys.exit(1)
            parser = get_parser([], None)
            parser.prog = "codefind"  # Set the program name on the parser
