...
VERSION_CHECK_FNAME = Path.home() / ".codefind" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of CodeFind from the main branch of the GitHub repository.
    """

    # Version updates disabled for CodeFind
    io.tool_output("Version updates are not available for CodeFind.")
    return False


def install_upgrade(io, latest_version=None):
    """
    Install the latest version of CodeFind from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer CodeFind version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of CodeFind?"

    docker_image = os.environ.get("CODEFIND_DOCKER_IMAGE")
    if docker_image:
        text = f"""
...
"""
    ...
    if success:
        io.tool_output("Re-run codefind to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/codefind/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = codefind.__version__
