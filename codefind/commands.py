...
from codefind import models, prompts
from codefind.editor import pipe_editor
from codefind.format_settings import format_settings
from codefind.help import Help, install_help_extra
from codefind.bedrock_knowledge_graph import create_client_from_args
from codefind.io import CommandCompletionException
# Removed litellm import - CodeFind uses direct Bedrock integration
from codefind.repo import ANY_GIT_ERROR
from codefind.run_cmd import run_cmd
# Web scraping removed - CodeFind focuses on Bedrock and knowledge graphs
from codefind.utils import is_image_file
...
class Commands:
    # Voice and web scraping removed - CodeFind focuses on Bedrock and knowledge graphs
    pass
    ...
    def __init__(
        ...
        # Voice functionality removed - CodeFind focuses on text-based Bedrock integration
    ...
    def cmd_chat_mode(self, args):
        ...
        show_formats = OrderedDict(
            [
                ("help", "Get help about using codefind (usage, config, troubleshoot)."),
    ...
    def completions_model(self):
        # Return Bedrock model names from our local metadata
        from codefind.models import model_info_manager
        models = [k for k in model_info_manager.local_model_metadata.keys()
    ...
    def cmd_web(self, args, return_content=False):
        "Web scraping disabled - CodeFind focuses on Bedrock and knowledge graphs"
        self.io.tool_error("Web scraping not supported in CodeFind. Use /knowledge-graph for context retrieval.")
        return
    ...
    def cmd_api_response(self, args):
        "Toggle display of detailed API response information including token usage"
        if not args.strip():
            # Show current status
            status = "enabled" if self.coder.show_api_response else "disabled"
            self.io.tool_output(f"API response display is currently {status}")
            self.io.tool_output("Use '/api-response on' to enable or '/api-response off' to disable")
            return

        arg = args.strip().lower()
        if arg in ['on', 'enable', 'true', '1']:
            self.coder.show_api_response = True
            self.io.tool_output("API response display enabled - detailed token usage will be shown after each API call")
        elif arg in ['off', 'disable', 'false', '0']:
            self.coder.show_api_response = False
            self.io.tool_output("API response display disabled")
        else:
            self.io.tool_error("Invalid argument. Use 'on' or 'off'")

    def cmd_undo(self, args):
        "Undo the last git commit if it was done by codefind"
        try:
    ...
    def raw_cmd_undo(self, args):
        ...
        if last_commit_hash not in self.coder.codefind_commit_hashes:
            self.io.tool_error("The last commit was not made by codefind in this chat session.")
            self.io.tool_output(
    ...
    def cmd_add(self, args):
        "Add files to the chat so codefind can edit them or review them in detail"
        ...
        for word in filenames:
            ...
            if self.coder.repo and self.coder.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} due to codefindignore or --subtree-only.")
                continue
    ...
    def basic_help(self):
        ...
        self.io.tool_output()
        self.io.tool_output("Use `/help <question>` to ask questions about how to use codefind.")

    def cmd_help(self, args):
        "Ask questions about codefind"
        ...
        from codefind.coders.base_coder import Coder
        ...
        # Start with standard help
        user_msg = self.help.ask(args)

        # Try to enhance with knowledge graph if available
        kg_client = create_client_from_args(self.args)
        if not kg_client:
            # Try to create from environment variables
            from codefind.bedrock_knowledge_graph import create_client_from_env
            kg_client = create_client_from_env()

        if kg_client:
            try:
                kg_results = kg_client.retrieve_and_format(args)
                if kg_results and "No results found" not in kg_results:
                    user_msg += f"\n\n# Additional context from knowledge graph:\n\n{kg_results}"
            except Exception as e:
                if self.verbose:
                    self.io.tool_output(f"Knowledge graph query failed: {e}")

        user_msg += """
# Announcement lines from when this session of codefind was launched:
...
"""
    ...
    def cmd_knowledge_graph(self, args):
        "Query AWS Bedrock Knowledge Graph for information"

        if not args.strip():
            self.io.tool_output("Please provide a query for the knowledge graph.")
            self.io.tool_output("Example: /knowledge-graph What is the architecture of this system?")
            return

        # Try to create knowledge graph client from args first, then from env
        kg_client = create_client_from_args(self.args)
        if not kg_client:
            # Try to create from environment variables
            from codefind.bedrock_knowledge_graph import create_client_from_env
        ...
        if not kg_client:
            self.io.tool_error("Knowledge graph not configured.")
            self.io.tool_output("To use knowledge graph queries, you need to set:")
            self.io.tool_output("  --bedrock-knowledge-base-id YOUR_KB_ID")
            self.io.tool_output("Or set environment variable: BEDROCK_KNOWLEDGE_BASE_ID")
            return

        try:
            # Test connection first
            if not kg_client.test_connection():
                self.io.tool_error("Unable to connect to knowledge graph.")
                self.io.tool_output("Please check your AWS credentials and knowledge base configuration.")
                return

            # Perform the query
            self.io.tool_output(f"Querying knowledge graph: {args}")
            results = kg_client.retrieve_and_format(args)

            # Display results
            self.io.tool_output(results)

            # Ask if user wants to add results to chat
            if results and "No results found" not in results:
                token_count = self.coder.main_model.token_count(results)
                k_tokens = token_count / 1000

                if self.io.confirm_ask(f"Add {k_tokens:.1f}k tokens of knowledge graph results to the chat?"):
                    self.coder.cur_messages += [
                        dict(role="user", content=f"Knowledge graph query: {args}"),
                        dict(role="assistant", content=results),
                    ]
                    self.io.tool_output("Knowledge graph results added to chat.")

        except Exception as e:
            self.io.tool_error(f"Knowledge graph query failed: {e}")
            if self.verbose:
                import traceback
                self.io.tool_output(traceback.format_exc())
...
"""
    ...
    def cmd_voice(self, args):
        "Voice input disabled - CodeFind focuses on text-based Bedrock integration"
        self.io.tool_error("Voice input not supported in CodeFind. Use text input or /knowledge-graph for context.")
        return
    ...
    def cmd_report(self, args):
        ...
        from codefind.report import report_github_issue
