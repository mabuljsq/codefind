...
from codefind import __version__, utils
from codefind.dump import dump  # noqa: F401
from codefind.help_pats import exclude_website_pats

warnings.simplefilter("ignore", category=FutureWarning)


def install_help_extra(io):
    # Help extras removed - CodeFind focuses on Bedrock and knowledge graphs
    io.tool_error("Interactive help not supported in CodeFind. Use --help for command-line help.")
    return False


def get_package_files():
    for path in importlib_resources.files("codefind.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    return f"https://codefind.ai/{url_path}"
...
def get_index():
    ...
    dname = Path.home() / ".codefind" / "caches" / ("help." + __version__)
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("codefind.website")
                .joinpath(fname)
