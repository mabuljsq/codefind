...
from codefind import __version__, models, utils
from codefind.args import get_parser
from codefind.coders import Coder
from codefind.coders.base_coder import UnknownEditFormat
from codefind.commands import Commands, SwitchCoder
from codefind.copypaste import ClipboardWatcher
# Deprecated model args removed - CodeFind only supports current Bedrock models
from codefind.format_settings import format_settings, scrub_sensitive_info
from codefind.history import ChatSummary
from codefind.io import InputOutput
from codefind.models import ModelSettings
from codefind.onboarding import select_default_model
from codefind.repo import ANY_GIT_ERROR, GitRepo
from codefind.report import report_uncaught_exceptions
from codefind.versioncheck import check_version, install_from_main_branch, install_upgrade
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run codefind in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track codefind's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".codefind"):
            patterns_to_add.append(".codefind*")
...
def check_streamlit_install(io):
    # Streamlit GUI removed - CodeFind focuses on command-line Bedrock integration
    io.tool_error("GUI mode not supported in CodeFind. Use command-line interface.")
    return False
...
def launch_gui(args):
    # Streamlit GUI removed - CodeFind focuses on command-line Bedrock integration
    print("GUI mode not supported in CodeFind. Use command-line interface.")
    return
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".codefind.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading CodeFind model settings: {e}")
        return 1
...
def discover_env_files(git_root, dotenv_fname, current_dir=None):
    """
    Dynamically discover .env files based on context and environment.

    Discovery order:
    1. OAuth keys file (~/.codefind/oauth-keys.env)
    2. Environment-specific files (.env.local, .env.development, .env.production, etc.)
    3. Standard search path (.env files in home, git root, current dir, command line)
    4. Directory traversal (.env files walking up from current directory)
    """
    ...
    # 1. OAuth keys file (highest priority)
    oauth_keys_file = Path.home() / ".codefind" / "oauth-keys.env"
    if oauth_keys_file.exists():
        dotenv_files.append(str(oauth_keys_file.resolve()))

    # 2. Environment-specific files based on common patterns
    env_variants = []

    # Check for NODE_ENV, ENVIRONMENT, or other common environment indicators
    env_name = os.environ.get('NODE_ENV') or os.environ.get('ENVIRONMENT') or os.environ.get('ENV')
    if env_name:
        env_variants.extend([f".env.{env_name}", f".env.{env_name}.local"])

    # Common environment variants
    common_envs = ['local', 'development', 'dev', 'staging', 'production', 'prod', 'test']
    for env in common_envs:
        env_variants.extend([f".env.{env}", f".env.{env}.local"])

    # Add environment-specific files from standard locations
    search_paths = []
    if git_root:
        search_paths.append(Path(git_root))
    search_paths.extend([current_dir, Path.home()])

    for search_path in search_paths:
        for variant in env_variants:
            env_file = search_path / variant
            if env_file.exists():
                dotenv_files.append(str(env_file.resolve()))

    # 3. Standard .env file search path
    standard_files = generate_search_path_list(".env", git_root, dotenv_fname)
    dotenv_files.extend(standard_files)

    # 4. Directory traversal - walk up from current directory to find .env files
    ...
    while current_path != current_path.parent:  # Stop at filesystem root
        ...
        if git_root and current_path == Path(git_root).resolve():
            break
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8", dynamic_discovery=True, verbose=False):
    """
    Load .env files with optional dynamic discovery.

    Args:
        git_root: Git repository root directory
        dotenv_fname: Specific .env file specified via command line
        encoding: File encoding to use
        dynamic_discovery: Whether to use dynamic discovery or legacy behavior
        verbose: Whether to print verbose discovery information
    """
    if dynamic_discovery:
        dotenv_files = discover_env_files(git_root, dotenv_fname)
        if verbose:
            print("Dynamic .env file discovery enabled")
            print("Discovered .env files in order:")
            for i, fname in enumerate(dotenv_files, 1):
                exists = "(exists)" if Path(fname).exists() else "(not found)"
                print(f"  {i}. {fname} {exists}")
    else:
        # Legacy behavior - standard search path only
        ...
        if verbose:
            print("Legacy .env file loading (dynamic discovery disabled)")
            print("Standard search path:")
            for i, fname in enumerate(dotenv_files, 1):
                exists = "(exists)" if Path(fname).exists() else "(not found)"
                print(f"  {i}. {fname} {exists}")
    ...
    if verbose and loaded:
        print(f"Successfully loaded {len(loaded)} .env file(s)")
    elif verbose:
        print("No .env files were loaded")

    return loaded


def register_bedrock_models_wrapper(git_root, model_metadata_fname, io, verbose=False):
    """Register Bedrock-only models from metadata files."""
    model_metadata_files = []

    # Add the resource file path
    resource_metadata = importlib_resources.files("codefind.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".codefind.model.metadata.json", git_root, model_metadata_fname
    )

    try:
        model_metadata_files_loaded = models.register_bedrock_models(model_metadata_files)
        if len(model_metadata_files_loaded) > 0 and verbose:
            io.tool_output("Loaded Bedrock model metadata from:")
            for model_metadata_file in model_metadata_files_loaded:
                io.tool_output(f"  - {model_metadata_file}")  # noqa: E221
    except Exception as e:
        io.tool_error(f"Error loading Bedrock model metadata: {e}")
        return 1
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("CodeFind only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run codefind --no-git to proceed without using git.")
        io.tool_output("See AWS CodeCommit documentation for git repository help.")
        return False
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".codefind.conf.yml")
    ...
    loaded_dotenvs = load_dotenv_files(
        ...
        args.env_file,
        args.encoding,
        dynamic_discovery=args.dynamic_env,
        verbose=args.env_discovery_verbose or args.verbose
    )
    ...
    if not args.verify_ssl:
        # SSL verification handled directly by boto3 for Bedrock
    ...
    register_bedrock_models_wrapper(git_root, args.model_metadata_file, io, verbose=args.verbose)

    # Check for knowledge graph dependencies if configured
    models.check_for_knowledge_graph_dependencies(io, args)
    ...
    if not selected_model_name:
        # Error message handled within select_default_model
    ...
    # Resolve any model aliases before validation
    resolved_model = models.MODEL_ALIASES.get(args.model, args.model)
    args.model = resolved_model

    # Validate that only Bedrock models are used (including inference profiles)
    ...
    if not is_bedrock_model(args.model):
        io.tool_error(f"Only AWS Bedrock models are supported. Model '{args.model}' is not a Bedrock model.")
        io.tool_output("Use models like: bedrock/anthropic.claude-3-5-sonnet-20240620-v1:0")
        io.tool_output("Or inference profiles like: us.anthropic.claude-3-5-sonnet-20240620-v1:0")
        io.tool_output("Run 'codefind --list-models bedrock/' to see available Bedrock models.")
        return 1
    ...
    # Deprecated settings removed - CodeFind only supports current configurations
    ...
    if args.show_model_warnings:
        ...
        if problem:
            ...
            try:
                io.tool_output("See AWS Bedrock documentation for model information.")
                io.tool_output()
    ...
    try:
        coder = Coder.create(
            ...
            show_api_response=args.show_api_response,
        ...
        io.tool_output("See CodeFind documentation for edit format information.")
        return 1
    ...
    if args.codefindignore:
        ignores.append(args.codefindignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from codefind.coders.base_coder import all_fences
    ...
    if args.show_release_notes is True:
        io.tool_output("CodeFind release notes available in project documentation")
        io.tool_output()
    elif args.show_release_notes is None and is_first_run:
        io.tool_output()
        io.tool_output("Welcome to CodeFind - AI pair programming with AWS Bedrock and knowledge graphs!")
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".codefind" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install CodeFind properly?")
                io.tool_output("Please ensure boto3 is installed: pip install boto3")
                sys.exit(1)
...
def load_slow_imports(swallow=True):
    ...
    try:
        ...
        import boto3  # noqa: F401  # For Bedrock integration
