...
from codefind import prompts, utils
...
class GitRepo:
    repo = None
    codefind_ignore_file = None
    codefind_ignore_spec = None
    codefind_ignore_ts = 0
    codefind_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if codefind_ignore_file:
            self.codefind_ignore_file = Path(codefind_ignore_file)

    def commit(self, fnames=None, context=None, message=None, codefind_edits=False, coder=None):
        """
        ...
        Args:
            ...
            codefind_edits (bool, optional): Whether the changes were made by CodeFind. Defaults to False.
        ...
        CodeFind generated the changes (`codefind_edits`).
        ...
        - codefind_edits=True: Changes were generated by CodeFind (LLM).
        - codefind_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (codefind)".
        - --attribute-committer: Modify Committer name to "User Name (codefind)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: codefind (<model>) <codefind@codefind.ai>" trailer to commit message.

        Behavior Summary:

        1. When codefind_edits = True (AI Changes):
        ...
        2. When codefind_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as CodeFind runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(codefind),
          Committer=You(codefind)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(codefind), Committer=You, Trailer added
        - User commit (defaults): codefind_edits=False -> Author=You, Committer=You(codefind)
        - User commit with explicit no-committer: codefind_edits=False,
        ...
        prefix_commit_message = codefind_edits and (
        ...
        if codefind_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: codefind ({model_name}) <codefind@codefind.ai>"

        # Determine if author/committer names should be modified
        # Author modification applies only to codefind edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            codefind_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of codefind_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an codefind edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (codefind_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "codefind: " + commit_message
        ...
        committer_name = f"{original_user_name} (codefind)"
    ...
    def refresh_codefind_ignore(self):
        if not self.codefind_ignore_file:
            return

        current_time = time.time()
        if current_time - self.codefind_ignore_last_check < 1:
            return

        self.codefind_ignore_last_check = current_time

        if not self.codefind_ignore_file.is_file():
            return

        mtime = self.codefind_ignore_file.stat().st_mtime
        if mtime != self.codefind_ignore_ts:
            self.codefind_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.codefind_ignore_file.read_text().splitlines()
            self.codefind_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.codefind_ignore_file or not self.codefind_ignore_file.is_file():
            return False
        ...
        return self.codefind_ignore_spec.match_file(fname)
