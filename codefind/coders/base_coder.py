...
from codefind import __version__, models, prompts, utils
from codefind.commands import Commands
from codefind.history import ChatSummary
from codefind.io import ConfirmGroup, InputOutput
from codefind.linter import Linter
from codefind import bedrock_llm
from codefind.models import RETRY_TIMEOUT
from codefind.reasoning_tags import (
    REASONING_TAG,
...
from codefind.repomap import RepoMap
from codefind.run_cmd import run_cmd
from codefind.utils import format_content, format_messages, format_tokens, is_image_file
from codefind.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_codefind_commit_hash = None
    codefind_edited_files = None
    ...
    # URL detection removed - CodeFind focuses on Bedrock and knowledge graphs
    ...
    def create(
        ...
        **kwargs,
        ...
        import codefind.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                codefind_commit_hashes=from_coder.codefind_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"CodeFind v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .codefindignore"
                )
                lines.append("See: https://docs.aws.amazon.com/bedrock/ for optimization tips")
        else:
    ...
    def __init__(
        ...
        self.event = lambda *args, **kwargs: None  # No-op event function
        self.chat_language = chat_language
        self.commit_language = commit_language
        self.commit_before_message = []
        self.codefind_commit_hashes = set()
        self.rejected_urls = set()
        self.abs_root_path_cache = {}

        self.auto_copy_context = auto_copy_context
        self.auto_accept_architect = auto_accept_architect
        self.show_api_response = show_api_response
        ...
        # URL detection disabled - CodeFind focuses on Bedrock and knowledge graphs
        ...
        if codefind_commit_hashes:
            self.codefind_commit_hashes = codefind_commit_hashes
        else:
            self.codefind_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches codefindignore spec.")
                continue
    ...
    def init_before_message(self):
        self.codefind_edited_files = set()
    ...
    def check_and_open_urls(self, exc, friendly_msg=None):
        """Display error messages - URL opening disabled in CodeFind."""
        text = str(exc)
    ...
    def check_for_urls(self, inp: str) -> str:
        """URL detection disabled - CodeFind focuses on Bedrock and knowledge graphs."""
        # Always return input unchanged - no URL processing
        return inp
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("CODEFIND_CACHE_KEEPALIVE_DELAY", delay))
        ...
        def warm_cache_worker():
            while self.ok_to_warm_cache:
                ...
                try:
                    completion = bedrock_llm.completion(
    ...
    def send_message(self, inp):
        ...
        from codefind.bedrock_llm import BedrockLLMError, BedrockRateLimitError, BedrockAuthError
        ...
        try:
            while True:
                try:
                    ...
                    # Handle Bedrock-specific exceptions directly
                    ex_info = type('ExInfo', (), {
                        'name': 'UnknownError',
                        'message': str(err),
                        'description': str(err),
                        'retry': False  # Don't retry by default for unknown errors
                    })()
        ...
        if edited:
            self.codefind_edited_files.update(edited)
    ...
    def show_exhausted_error(self):
        ...
        self.io.tool_output("See AWS Bedrock documentation for token limits: https://docs.aws.amazon.com/bedrock/")
    ...
    def show_send_output(self, completion):
        ...
        # Print API response with token usage information if enabled
        if self.show_api_response:
            self._print_api_response_with_tokens(completion)
    ...
    def calculate_bedrock_cost(self, completion):
        """Calculate cost for Bedrock models based on token usage."""
        # Simplified cost calculation for Bedrock models
        # In a real implementation, you'd use actual Bedrock pricing
        try:
            input_tokens = getattr(completion, 'usage', {}).get('prompt_tokens', 0)
            output_tokens = getattr(completion, 'usage', {}).get('completion_tokens', 0)

            # Approximate costs (these should be updated with actual Bedrock pricing)
            input_cost_per_token = 0.000003  # $3 per 1M tokens (example)
            output_cost_per_token = 0.000015  # $15 per 1M tokens (example)
    ...
    def calculate_and_show_tokens_and_cost(self, messages, completion=None):
        ...
        try:
            # Use Bedrock-specific cost calculation
            cost = self.calculate_bedrock_cost(completion)
        except Exception:
            cost = 0
    ...
    def _print_api_response_with_tokens(self, completion):
        """Print API response details including token usage information."""
        if not completion:
            return

        self.io.tool_output("=" * 60)
        self.io.tool_output("API RESPONSE DETAILS")
        self.io.tool_output("=" * 60)

        # Print model information
        if hasattr(completion, 'model'):
            self.io.tool_output(f"Model: {completion.model}")

        # Print token usage information
        if hasattr(completion, 'usage') and completion.usage:
            usage = completion.usage
            self.io.tool_output("TOKEN USAGE:")

            if hasattr(usage, 'prompt_tokens'):
                self.io.tool_output(f"  Input tokens: {usage.prompt_tokens:,}")
            if hasattr(usage, 'completion_tokens'):
                self.io.tool_output(f"  Output tokens: {usage.completion_tokens:,}")
            if hasattr(usage, 'total_tokens'):
                self.io.tool_output(f"  Total tokens: {usage.total_tokens:,}")

            # Cache-related tokens
            cache_hit = getattr(usage, 'prompt_cache_hit_tokens', 0) or getattr(usage, 'cache_read_input_tokens', 0)
            cache_write = getattr(usage, 'cache_creation_input_tokens', 0)

            if cache_hit > 0:
                self.io.tool_output(f"  Cache hit tokens: {cache_hit:,}")
            if cache_write > 0:
                self.io.tool_output(f"  Cache write tokens: {cache_write:,}")
        else:
            self.io.tool_output("TOKEN USAGE: Not available in response")

        # Print response metadata
        if hasattr(completion, 'choices') and completion.choices:
            choice = completion.choices[0]
            if hasattr(choice, 'finish_reason'):
                self.io.tool_output(f"Finish reason: {choice.finish_reason}")

        # Print raw response structure (abbreviated)
        self.io.tool_output("RAW RESPONSE STRUCTURE:")
        try:
            import json
            # Create a simplified version of the response for display
            response_dict = {}
            if hasattr(completion, 'model'):
                response_dict['model'] = completion.model
            if hasattr(completion, 'usage') and completion.usage:
                response_dict['usage'] = {
            ...
            if hasattr(completion, 'choices') and completion.choices:
                response_dict['choices'] = [{
                    ...
                    'message': {
                        'role': 'assistant',
                        'content': f"[{len(getattr(completion.choices[0].message, 'content', '') or '')} characters]"
                    }
                }]

            self.io.tool_output(json.dumps(response_dict, indent=2))
        except Exception as e:
            self.io.tool_output(f"Could not serialize response: {e}")
    ...
    def check_added_files(self):
        ...
        self.io.tool_warning("See AWS Bedrock documentation for troubleshooting: https://docs.aws.amazon.com/bedrock/")
    ...
    def apply_updates(self):
        ...
        try:
            ...
            self.io.tool_output("See AWS Bedrock documentation for troubleshooting: https://docs.aws.amazon.com/bedrock/")
            self.io.tool_output()
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, codefind_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_codefind_commit_hash = commit_hash
        self.codefind_commit_hashes.add(commit_hash)
        self.last_codefind_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each CodeFind commit.")
