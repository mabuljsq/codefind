...
# datetime import removed - no longer needed without GitHub Copilot integration
...
# __version__ import removed - no longer needed without GitHub Copilot integration
from codefind.dump import dump  # noqa: F401
from codefind import bedrock_llm
# Only Bedrock models supported - no OpenRouter integration needed
from codefind.sendchat import ensure_alternating_roles, sanity_check_messages
from codefind.utils import check_pip_install_extra

RETRY_TIMEOUT = 60

request_timeout = 600

DEFAULT_MODEL_NAME = "bedrock/us.anthropic.claude-3-7-sonnet-20250109-v1:0"

# Mapping of model aliases to their canonical names
# The system will automatically use inference profiles when available
MODEL_ALIASES = {
    # Bedrock Claude models (latest) - system will find best available version
    "sonnet": "bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0",  # Latest Sonnet
    "sonnet35": "bedrock/anthropic.claude-3-5-sonnet-20241022-v2:0",  # Claude 3.5 Sonnet v2
    "sonnet35-old": "bedrock/anthropic.claude-3-5-sonnet-20240620-v1:0",  # Claude 3.5 Sonnet v1
    "haiku": "bedrock/anthropic.claude-3-5-haiku-20241022-v1:0",
    "opus": "bedrock/anthropic.claude-3-opus-20240229-v1:0",
    # Bedrock other models
    "titan": "bedrock/amazon.titan-text-premier-v1:0",
    "llama90b": "bedrock/meta.llama3-2-90b-instruct-v1:0",
    "llama11b": "bedrock/meta.llama3-2-11b-instruct-v1:0",
    "llama3b": "bedrock/meta.llama3-2-3b-instruct-v1:0",
    "llama1b": "bedrock/meta.llama3-2-1b-instruct-v1:0",
}
...
class ModelSettings:
    ...
    actual_name: Optional[str] = None  # For model aliases


# Load model settings from package resource
MODEL_SETTINGS = []
with importlib.resources.open_text("codefind.resources", "model-settings.yml") as f:
...
class ModelInfoManager:
    # LiteLLM URL removed - CodeFind only uses local Bedrock model metadata
    CACHE_TTL = 60 * 60 * 24  # 24 hours

    def __init__(self):
        self.cache_dir = Path.home() / ".codefind" / "caches"
    ...
    def get_model_info(self, model):
        ...
        if not cached_info:
            try:
                bedrock_info = bedrock_llm.get_model_info(model)
            except Exception as ex:
                print(str(ex))
...
class Model(ModelSettings):
    def __init__(
        ...
        self.extra_model_settings = next(
            (ms for ms in MODEL_SETTINGS if ms.name == "codefind/extra_params"), None
        )
        ...
        # Check if streaming is disabled via environment variable (override any model settings)
        import os
        if os.environ.get('BEDROCK_DISABLE_STREAMING', 'false').lower() == 'true':
    ...
    def configure_model_settings(self, model):
        ...
        if (
            ...
            and self.extra_model_settings.name == "codefind/extra_params"
        ):
    ...
    def tokenizer(self, text):
        return bedrock_llm.encode(model=self.name, text=text)

    def token_count(self, messages):
        if type(messages) is list:
            try:
                return bedrock_llm.token_counter(model=self.name, messages=messages)
    ...
    def fast_validate_environment(self):
        """Fast path for Bedrock models. Avoids forcing litellm import."""

        model = self.name

        # Only support Bedrock models and inference profiles
        ...
        if is_bedrock_model(model):
            # AWS credentials are handled via standard AWS credential chain
            ...
            if any(os.environ.get(cred) for cred in aws_creds):
                return dict(keys_in_environment=["AWS_CREDENTIALS"], missing_keys=[])
    ...
    def validate_environment(self):
        ...
        # Use direct Bedrock validation instead of litellm

        model = self.name
        res = bedrock_llm.validate_environment(model)
        ...
        # Only Bedrock models are supported
        provider = self.info.get("litellm_provider", "").lower()
        if provider != "bedrock":
            return dict(keys_in_environment=[], missing_keys=["AWS_CREDENTIALS"])
    ...
    # GitHub Copilot integration removed - CodeFind only supports AWS Bedrock

    def send_completion(self, messages, functions, stream, temperature=None):
        if os.environ.get("CODEFIND_SANITY_CHECK_TURNS"):
        ...
        # CodeFind only supports AWS Bedrock - no GitHub Copilot integration needed

        res = bedrock_llm.completion(**kwargs)
    ...
    def simple_send_with_retries(self, messages):
        ...
        while True:
            try:
                ...
                from codefind.reasoning_tags import remove_reasoning_content

                return remove_reasoning_content(res, self.reasoning_tag)

            except BedrockRateLimitError as err:
...
def register_bedrock_models(model_fnames):
    """
    Register Bedrock-only models from metadata files.
    Simplified version that only handles Bedrock models.
    """
    files_loaded = []
    for model_fname in model_fnames:
        ...
        try:
            ...
            # Only register Bedrock models
            bedrock_models = {k: v for k, v in model_def.items()
                            if k.startswith("bedrock/") and v.get("litellm_provider") == "bedrock"}

            model_info_manager.local_model_metadata.update(bedrock_models)
        except Exception as e:
            raise Exception(f"Error loading Bedrock model definition from {model_fname}: {e}")
...
def check_for_dependencies(io, model_name):
    ...
    """
    # Check if this is a Bedrock model or inference profile and ensure boto3 is installed
    def is_bedrock_model(model_name):
        """Check if model is a Bedrock model or inference profile."""
        return (model_name.startswith("bedrock/") or
                any(model_name.startswith(f"{region}.") for region in
    ...
    if is_bedrock_model(model_name):
        ...
        # Only Bedrock models are supported
        io.tool_error(f"Model '{model_name}' is not supported. Only AWS Bedrock models are supported.")
...
def check_for_knowledge_graph_dependencies(io, args):
    """
    Check for AWS Bedrock Knowledge Graph dependencies if configured.
    ...
    if (hasattr(args, 'bedrock_knowledge_base_id') and args.bedrock_knowledge_base_id) or \
       os.environ.get('BEDROCK_KNOWLEDGE_BASE_ID'):
        check_pip_install_extra(
            io,
            "boto3",
            "AWS Bedrock Knowledge Graph requires the boto3 package.",
            ["boto3"]
        )


def fuzzy_match_models(name):
    """Fuzzy match Bedrock models only."""
    name = name.lower()

    chat_models = set()
    # Only search in local Bedrock model metadata - no litellm.model_cost
    ...
    for orig_model, attrs in model_metadata:
        ...
        # Only match Bedrock models
        if provider != "bedrock":
            continue

        # Add the model as-is (already has bedrock/ prefix)
